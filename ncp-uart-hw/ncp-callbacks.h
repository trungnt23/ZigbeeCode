...
/** EZSP Command setConcentrator(EZSP_SET_CONCENTRATOR)
 *
 ...
 * The former is used when the caller has enough memory to store source routes
 * for the whole network. In that case, remote nodes stop sending route records
 * once the concentrator has successfully received one. The latter is used when
 * the concentrator has insufficient RAM to store all outbound source routes.
 * In that case, route records are sent to the concentrator prior to every
 * inbound APS unicast.
 * @param minTime
 ...
 * @param routeErrorThreshold
 * The number of route errors that will trigger a re-broadcast of the MTORR.
 * @param deliveryFailureThreshold
 * The number of APS delivery failures that will trigger a re-broadcast of the
 * MTORR.
 * @param maxHops
...
/** EZSP Command sendRawMessageExtended(EZSP_SEND_RAW_MESSAGE_EXTENDED)
 *
 * Transmits the given message without modification. The MAC header is assumed
 * to be configured in the message at the time this function is called.
 *
...
/** EZSP Command setSourceRouteDiscoveryMode(EZSP_SET_SOURCE_ROUTE_DISCOVERY_MODE)
 *
 ...
 *
 * @return
 * Remaining time(ms) until next MTORR broadcast if the mode is on,
